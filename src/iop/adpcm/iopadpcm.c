...
void IAdpcmPreLoad(ADPCM_CMD *acp)
{
    ...
    ICdvdLoadReqAdpcm(iop_adpcm[channel].start, acp->size, AdpcmIopBuf[channel], channel, 1u, endld_flg);
}
...
static void FillBuffer(int size, u_char channel, s16** dec_buf)
{
    ...
    for (int i = 0; i < chunks; i++)
    {
        ...
        SDL_PutAudioStreamPlanarData(iop_adpcm[channel].stream, (void *) dec, 2, 3584);
    }
}
...
void IAdpcmPlay(ADPCM_CMD *acp)
{
    ...
    if (iop_adpcm[channel].stat == ADPCM_STAT_PRELOAD_END)
    {
        ...
        SDL_SetAudioStreamGetCallback(iop_adpcm[channel].stream, IAdpcmReadCh0, NULL);
        SDL_SetAudioStreamFrequencyRatio(iop_adpcm[channel].stream, (float) acp->pitch / (float) 0x1000);
                ...
                if (iop_adpcm[0].pos == (iop_adpcm[0].dbidi + 1) * 0x20000) {
                    ...
                    if (remain_l > 0x20000) {
                        ...
                        ICdvdLoadReqAdpcm(
                            ...
                            (s16*)&AdpcmIopBuf[0][0x20000 * iop_adpcm[0].dbidi], 0, 2u, 0);
                    } else if (remain_l) {
                        iop_adpcm[0].lreq_size = remain_l;
                        ICdvdLoadReqAdpcm(
                            start,
                            iop_adpcm[0].lreq_size,
                            (s16*)&AdpcmIopBuf[0][0x20000 * iop_adpcm[0].dbidi], 0, 2u, 1u);
                    }
